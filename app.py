...
if uploaded_file:
    ...
    # ⚠️ Correction : ignorer la toute première valeur (conso de 23h55 → 00h00 de la veille)
    if not df.empty:
        df.iloc[0, df.columns.get_loc("Valeur")] = pd.NA

    # 4. Vérification des bornes
    debut_brut, fin_brut = df["Horodate"].min(), df["Horodate"].max()
    pas_temps = df["Horodate"].diff().min()
    st.info(f"📅 Données disponibles : du **{debut_brut.strftime('%d/%m/%Y %H:%M')}** "
            f"au **{fin_brut.strftime('%d/%m/%Y %H:%M')}**")
    st.info(f"⏱ Pas de temps détecté : {pas_temps}")
    ...
    if st.button("🚀 Lancer le traitement"):
        ...
        # 9. Détection changements d’heure
